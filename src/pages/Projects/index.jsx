...
export default function Projects({ title }) {
  ...
  const handleGetRepositories = async () => {
    const data = await fetch(
      "https://api.github.com/users/edwardribas/repos"
    ).then((e) => e.json());
    setIsLoading(false);
    setRepositories(
      data.filter((proj) => proj.topics.includes("to-portfolio"))
    );
  };
        ...
        <div id={styles.projects}>
          ...
          {!isLoading &&
            repositories.map((proj, i) => {
              ...
              const searchQueryIncludeProject = proj.description
                .split(" - ")[0]
                .toLowerCase()
                .includes(query.toLowerCase());
              const projectStack = topics
                .filter((e) => e.startsWith("tec-"))
                .map((e) => {
